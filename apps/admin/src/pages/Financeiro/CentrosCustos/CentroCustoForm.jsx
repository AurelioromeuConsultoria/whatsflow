...
export default function CentroCustoForm() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(t('finance.costCenters.saveError'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(isEditing ? t('finance.costCenters.saveSuccessEdit') : t('finance.costCenters.saveSuccessCreate'));
      navigate('/financeiro/centros-custos');
    } catch (err) {
      const errorMessage = err.response?.data?.message || t('finance.costCenters.saveError');
  ...
  if (loading && isEditing) return <LoadingPage text={t('finance.costCenters.loadingForm')} />;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">{isEditing ? t('finance.costCenters.editTitle') : t('finance.costCenters.newTitle')}</h1>
          <p className="text-muted-foreground">{isEditing ? t('finance.costCenters.editSubtitle') : t('finance.costCenters.newSubtitle')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>{t('finance.costCenters.cardTitle')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2 flex items-center space-x-3">
                <input
                  ...
                  className="h-4 w-4"
                />
                <Label htmlFor="ativo" className="cursor-pointer">{t('finance.costCenters.activeLabel')}</Label>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex items-center space-x-4">
          ...
          <Button type="button" variant="outline" asChild>
            <Link to="/financeiro/centros-custos">{t('actions.cancel')}</Link>
          </Button>
        </div>
      </form>
    </div>
